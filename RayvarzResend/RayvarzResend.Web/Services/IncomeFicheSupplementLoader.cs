...
public static class IncomeFicheSupplementLoader
{
    public static async Task EnrichAsync(
        ...
        await EnrichAsync(fiche, saraConnectionString, null, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        if (prior.NidIncome != Guid.Empty && loadCalculationRowsAsync is not null)
        ...
        CancellationToken ct)
    {
        ...
        var sql = $"""
            SELECT TOP (1)
                   ...
                   f.EumFicheStatus AS FicheStatus
        ...
        try
        {
            ...
            return new PriorIncomeFicheDto
            {
                ...
                FicheStatus = ReadInt32(reader, "FicheStatus")
            };
        }
        catch (SqlException)
        {
    ...
    private static int ResolveDistrictBranch(FicheHeaderDto fiche) =>
        Member1388IncomeCenterResolver.ResolveDistrictBranch(fiche);
