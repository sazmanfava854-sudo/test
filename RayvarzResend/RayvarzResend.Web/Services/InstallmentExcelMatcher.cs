...
public static class InstallmentExcelMatcher
{
    ...
    {
        "NoDocument", "trackingno", "PaymentCost", "PaymentDate"
    };
    ...
    public static bool TrackingMatches(string? excelTracking, string? dbTracking)
    {
        var excel = NormalizeDigits(excelTracking);
        if (string.IsNullOrEmpty(excel))
            return true;
        var db = NormalizeDigits(dbTracking);
        return string.Equals(excel, db, StringComparison.OrdinalIgnoreCase);
    }

    public static (InstallmentLookupKind Kind, string Value, string? Error) ResolveLookup(InstallmentExcelRowInput row)
    {
        var noDocument = NormalizeDigits(row.NoDocument);
        var trackingNo = NormalizeDigits(row.TrackingNo);

        if (!string.IsNullOrEmpty(noDocument))
            return (InstallmentLookupKind.NoDocument, noDocument, null);
        if (!string.IsNullOrEmpty(trackingNo))
            return (InstallmentLookupKind.TrackingNo, trackingNo, null);

        return (InstallmentLookupKind.NoDocument, "", "NoDocument یا TrackingNo الزامی است");
    }

    public static string? ValidateAgainstDb(InstallmentExcelRowInput excel, InstallmentRowSnapshot db)
    {
        ...
        if (!TrackingMatches(excel.TrackingNo, db.TrackingNo))
            return "کد پیگیری (trackingno) با دیتابیس مطابقت ندارد";

        var excelNoDoc = NormalizeDigits(excel.NoDocument);
        if (!string.IsNullOrEmpty(excelNoDoc))
        {
            var dbNoDoc = NormalizeDigits(db.NoDocument);
            if (!string.Equals(excelNoDoc, dbNoDoc, StringComparison.OrdinalIgnoreCase))
